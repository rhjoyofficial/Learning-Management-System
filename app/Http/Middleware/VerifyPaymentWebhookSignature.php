...
{
    /**
     * Handle an incoming request for generic payment webhooks.
     *
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (!$secret) {
            \Log::warning('Payment webhook secret not configured');
        ...
        if (!$signature) {
            \Log::warning('Payment webhook signature missing', [
                'ip' => $request->ip(),
                'url' => $request->fullUrl(),
            ]);
        ...
        if (!hash_equals($expectedSignature, $signature)) {
            \Log::error('Payment webhook signature mismatch', [
                'ip' => $request->ip(),
